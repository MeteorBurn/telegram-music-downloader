...
- Type/extension/size/date filters run before queueing. `filters.duration` runs after
  downloading, before normalization and tracking, using `ffprobe` found on `PATH`.
  Both duration limits unset means no probe. Out-of-range files are deleted and skipped;
  probe or deletion errors return `failed`. Existing-file skip paths bypass this check.
- Skip checks use the blacklist, tracked message/path, predicted normalized path and
  ordinary destination path. Existing files can be registered without re-downloading;
  existence alone is not a completeness check. No byte-offset resume is implemented here.
- `normalize_track_names` defaults to false. Cleanup removes a trailing `__<message_id>`
  and selected tags, so names can collide. Preserve existing-file checks and tracker paths.
...
| `tests/test_runtime.py` | Channel queueing/checkpoints, request conversion, duration rejection, normalization collision, retries, shutdown, locator |
